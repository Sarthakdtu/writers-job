...
export const HomeView = () => {
  ...
  const handleSoftDelete = async (storyId) => {
    const story = stories.find((s) => s.id === storyId);
    if (!confirm(`Move "${story?.title || storyId}" to trash? You can restore it later.`)) return;
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div>
            <div className="flex items-center gap-2 text-xs font-bold text-[var(--accent)] uppercase tracking-wider mb-1">
              ...
              <span>Home · Your Writing Universe</span>
            </div>
            ...
            <p className="text-sm text-[var(--text-muted)] mt-2 max-w-xl">
              Pick a story to open its dedicated dashboard for the overview, aesthetic settings and
              a random corner of fun facts.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 rounded-xl border border-[var(--border-color)] bg-[var(--bg-card)] px-4 py-2.5 text-xs font-semibold text-[var(--text-muted)] hover:bg-[var(--bg-hover)] hover:text-[var(--text-main)] transition-all cursor-pointer"
            >
                ...
                <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                  <div>
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <button
                        onClick={() => handleSoftDelete(story.id)}
                        className="shrink-0 rounded-lg p-1.5 text-[var(--text-dim)] hover:bg-[var(--bg-hover)] hover:text-red-500 transition-colors"
                        title="Move to trash"
                      >
                  ...
                  <button
                    ...
                    className={`w-full flex items-center justify-center gap-1.5 rounded-xl py-2 text-xs font-semibold transition-all cursor-pointer ${
                      isSelected
                        ? 'bg-[var(--accent)] text-white'
